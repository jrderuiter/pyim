...
class TestFlattenOptions(object):
    def test_simple(self):
        """Tests a basic set of options."""

        options = OrderedDict([('--opt1', 'a'), ('--opt2', 'b')])
        result = util.flatten_options(options)
    ...
    def test_non_str_type(self):
        ...
        options = {'--opt1': 1}
        assert util.flatten_options(options) == ['--opt1', '1']
    ...
    def test_flag_true(self):
        ...
        options = {'--opt1': True}
        assert util.flatten_options(options) == ['--opt1']

    def test_flag_false(self):
        """Tests options with negative flag."""

        options = {'--opt1': False}
        assert util.flatten_options(options) == []
